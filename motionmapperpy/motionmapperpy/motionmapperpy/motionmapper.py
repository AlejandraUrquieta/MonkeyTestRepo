...
def file_embeddingSubSampling(projectionFile, parameters, VERSION):
    ...
    try:
        ...
        # 1) Load projections
            assert False, 'missing: first save the data into the projectionFile locaiton. Then modify parameters to load fro this file.'
            # the result should be a list of arrays
        # 2) Check shpa eof projections
        if isinstance(projections_in, list):
            VERSION = 'monkey'
        else:
            VERSION = 'default'
            projections = projections_in
    except:
    ...
    if VERSION=="default":
        signalAmps, signalData = get_wavelet(projections)
    elif VERSION=='monkey':
        # Do this in a loop, once for each projection in the list
        list_sa = []
        list_sd = []
        for projections in projections_in:
            sa, sd = get_waveled(projections)
            list_sa.append(sa)
            list_sd.append(sd)
        # concatenate
        # list_sd should all have the 2nd dimeinsios
        # [(N1, D), (N2, D), (N3, D)]
        ...
        signalAmps = np.concat(list_sa, axis=0)
        signalData = np.concat(list_sd, axis=0)
