...
def file_embeddingSubSampling(projectionFile, parameters):
    ...
    try:
        ...
        if isinstance(projections_in, numpy.ndarray):
            VERSION = 'monkey'
            print('monkey')
        else:
            VERSION = 'default'
    ...
    if VERSION=="default":
        ...
        for projections in projections_in:
            sa, sd, signalIdx = get_wavelet(projections, parameters)
            ...
            '''
            #sa[]
            #for x,inds in [sa, sd],(len(sa)-1):
                #assert len(x.shape)==len(x[inds].shape), "failed... diff shapes"
